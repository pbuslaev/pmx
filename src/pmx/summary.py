...
def compare_datasets_arrays( valsArr1,valsArr2,errsArr1=[],errsArr2=[], func='aue', bootnum=1000,
    ...
    ######## calc value #######
    if 'aue' in func:
        val = _calc_aue( valsArr1, valsArr2 )
    elif 'rmse' in func:
        val = _calc_rmse( valsArr1, valsArr2 )
    elif 'pearson' in func:
        val = _calc_pearson( valsArr1, valsArr2 )      
    elif 'kendall' in func:
        val = _calc_kendall( valsArr1, valsArr2 )                
    ...
    for boot in range(0,bootnum):
        ...
        fooval = 0.0
        if 'aue' in func:
            fooval = _calc_aue( d1[rand], d2[rand] )
        elif 'rmse' in func:
            fooval = _calc_rmse( d1[rand], d2[rand] )
        elif 'pearson' in func:
            fooval = _calc_pearson( d1[rand], d2[rand] )            
        elif 'kendall' in func:
            fooval = _calc_kendall( d1[rand], d2[rand] )                    
        err.append( fooval )
...
def _calc_aue( arr1, arr2 ):
    val = np.mean(np.abs(arr1-arr2))
    return(val)

def _calc_rmse( arr1, arr2 ):
    val = np.sqrt( np.mean( np.power(arr1-arr2,2.0) ) )
    return(val)

def _calc_pearson( arr1, arr2 ):
    val = np.corrcoef(arr1,arr2)[0,1]
    return(val)

def _calc_kendall( arr1, arr2 ):
